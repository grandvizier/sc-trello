...
app.locals.strTrunc = (n, useWordBoundary) ->
  isTooLong = @length > n
  s_ = if isTooLong then @substr(0, n - 1) else this
  s_ = if useWordBoundary and isTooLong then s_.substr(0, s_.lastIndexOf(' ')) else s_
  if isTooLong then s_ + '...' else s_
